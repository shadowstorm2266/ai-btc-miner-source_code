...
class RealMinerEnvV3_5(gym.Env):
    def __init__(self, block_headers, target_difficulty=2**240):
        super(RealMinerEnvV3_5, self).__init__()
    ...
    def step(self, action):
        # Add slight entropy to action
        nonce = (action + random.randint(0, 5)) % self.nonce_range
        ...
        if hash_int < self.target_difficulty:
            reward = 5
        elif hash_int < self.target_difficulty * 2:
            reward = 0.5
        else:
            reward = -0.05
